...
plot_path = "../Load/01-07-2024/group2/"

fileToPlot = "output-intensity_profile-three-21-06-2024-10min-100.0requests.csv,-restart-cluster112.csv"
...
save_path = f"../01-07-2024/data_15-{cpu_step}/"
...
def plot_json(file_name, label):
    ...
    if len(json_data['data']['result']) > 0:
        ...
        plt.plot(greater_than_valueReduce, lissageValues,'o-', color=color, label=label)
...
#save_path = "../21-06-2024/data_18-25/"
...
for file_name in os.listdir(directory2):
    ...
    last_part = (file_parts[-1]).split(".")[0]
    result = re.split(r'-\d+', last_part)[0]
    timestamps2 = plot_json(file_path, last_part)
...
my_string = f"{save_graphics_at}/output{str(data_count + 1)}-{cpu_step}min.png"
